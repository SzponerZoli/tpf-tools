...
def select_tool(new_tool):
    """Kiválasztja az eszközt (pen, eraser vagy fill)."""
    global tool
    tool = new_tool

def fill_area(event):
    """A terület kitöltése a kiválasztott színnel (vödör eszközzel)."""
    x, y = event.x, event.y
    target_color = current_image.getpixel((x, y))
    if target_color != hex_to_rgb(current_color):
        fill_bucket(x, y, target_color, hex_to_rgb(current_color))


def hex_to_rgb(hex_color):
    """Átalakítja a hex színt RGB színformátummá."""
...
canvas.bind("<Button-3>", fill_area)  # Jobb egérgomb a vödör eszközhöz
